...
- [x] 🟡 물고기 보관함 & 수조 배치 시스템 (기획서 5.2.7) — 꾸미기 인벤토리 패턴 차용
  ...
  - [x] 좌측 하단 패널 UX — 인큐베이터(🥚)·보관함(📦)·짝짓기(💞) 상호 배타(하나 열면 다른 하나 닫힘) + 열린 패널 z-index 상향 (제어형 컴포넌트로 전환, leftPanel 단일 상태)
- [x] 🔴 번식(짝짓기) 시스템
  - [x] 자격 판정 (`utils/breeding.ts` — 같은 종 + 성어/대형어 + 쿨다운) `isBreedable`/`canPair`
  - [x] 비용·쿨다운 (`BREED_COST_PEARL` 200, `BREED_COOLDOWN_SECONDS` 12h, `Fish.lastBredAt`) — 부모 소멸 없음
  - [x] 번식 알 파이프라인 재사용 (`Egg.breedSpeciesId` → 인큐베이터 → 부화), 부모 등급별 알 tier
  - [x] 부화 추첨 규칙 (`rollBreedingSpecies` — 같은 종, 8% 상위 등급) 클라·서버 동일
  - [x] 서버 권위 (`breedFish` onCall: Pearl 차감·부모 쿨다운·알 지급 트랜잭션) + 낙관적 UI/롤백
  - [x] 짝짓기 패널 UI (`BreedingPanel` — 부모 2슬롯·후보 목록·쿨다운 표시) + `breed_fish` 분석 이벤트
- [-] 🔴 가챠 시스템 (알)
  - [x] 알 3티어 정의 (기본/희귀/전설)
...
- [ ] 🔴 베타 테스터 500명 모집
  ...
  - [ ] 테스터 NDA 동의 프로세스
- [x] 🔴 피드백 수집 채널 구축
  - [x] 인앱 피드백 버튼 구현 (설정 → 의견 보내기 — 유형 분류(버그/제안/기타) + 본문 + 답변 이메일, 환경정보(플랫폼/버전/화면/UA) 자동 첨부, `submitFeedback` Cloud Function → `feedback` 컬렉션 적재, 게스트 포함 제출, firestore.rules 클라 직접 접근 차단)
...
> **참고:** 기본 번식(짝짓기) 시스템은 MVP(2-3)에서 이미 출시됨 — 같은 종 성어 2마리 → Pearl 비용 + 12h 쿨다운 → 번식 알(같은 종, 8% 상위 등급). V1.2 는 그 위에 **유전·돌연변이 깊이**를 얹는다.

- [x] 🔴 교배 UI 및 인터랙션 — `BreedingPanel`(부모 2슬롯·후보·쿨다운) *(MVP 완료)*
  - [ ] 교배 결과 예측 미리보기 (색상 조합 힌트) *(유전 시스템 이후)*
  - [ ] 교배 연출 애니메이션 (전용 — 현재는 부화 연출 재사용)
- [ ] 🔴 유전자 시스템 설계 및 구현
  - [ ] 색상 유전 로직 (부모 색상 조합) — `Fish.color` 유전
  - [ ] 무늬 유전 로직
  - [ ] 돌연변이 확률 (1~5%) — 변이 색상/반짝이 개체
  - [ ] 돌연변이 종 DB 구성 (희귀 외형 20종 이상)
- [x] 🔴 교배 쿨다운 및 제한 로직 — 부모별 12h 쿨다운 + Pearl 비용 *(MVP 완료)*
